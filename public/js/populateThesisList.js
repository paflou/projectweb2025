...
updateButton.addEventListener('click', async function (e) {
    ...
    const response = await fetch('/prof/api/update-topic', {
        method: 'POST',
...
publishButton.addEventListener('click', async function (e) {
    ...
    // Send updated data to the server
    const response = await fetch('/prof/api/create-topic', {
        method: 'POST',
        body: formData // FormData handles multipart/form-data automatically
    });
...
async function populateThesisTable() {
    const response = await fetch('/prof/api/get-under-assignment', {
        method: 'GET',
        headers: {
    ...
    if (info.length === 0) {
        ...
        for (let i = 0; i < info.length; i++) {
            ...
            newDeleteButton.addEventListener('click', async () => {
                ...
                const response = await fetch('/prof/api/delete-topic', {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                ...
                if (response.ok) {
                    ...
                    showSuccess("Το θέμα διαγράφτηκε επιτυχώς")
                } else {
                    console.error('Failed to delete thesis');
                    showError("Προέκυψε σφάλμα")
                }
            }
            );
        }
